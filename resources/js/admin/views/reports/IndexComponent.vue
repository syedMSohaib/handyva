                    ...
                    <div class="card-content collapse show">
                        <div class="card-body card-dashboard">
                            ...
                            <div class="maain-tabble mt-3  table-responsive">
                                <table class="table table-striped table-bordered zero-configuration" id="reportTable">
                                    <thead>
                                        <tr>
                                            <th>S.No</th>
                                            <th>Date created</th>
                                    ...
                                    <tbody>
                                        <tr v-bind:key="index" v-for="(row, index) in data">
                                            ...
                                            <td>{{ row.reportable.id }}</td>
                                            <td class="name-td">
                                                <img class="avatar"
                                                    :src="row.reportable.image">&nbsp;&nbsp;<span>{{ row.reportable.name }}</span>
                                            </td>
                                            <td>{{ row.reportable.email }}</td>
                                                ...
                                                <div class="btn-group mr-1 mb-1">
                                                    ...
                                                    <div class="dropdown-menu" x-placement="bottom-start"
                                                        ...
                                                        <router-link class="dropdown-item"
                                                            :to="{ name: 'report.show', params: { id: row.id } }">
    ...
    export default {
        ...
        methods: {
            getReports() {
                ...
                axios.get(`/reports?&from=${this.dateFormat(this.from)}&to=${this.dateFormat(this.to)}`)
                    .then(data => {
                        this.data = data.data.data;
                        setTimeout(() => this.table = $('#reportTable').DataTable(), 100);
                    });
            },
        },
        watch: {
