        ...
        <div class="top">
            <div class="row align-items-center mt-2">
                ...
                <div
                    ...
                    <router-link v-if="blocked !== 0" :to="{ name: 'usermg.org.verify'}" class="blue-btn d-inline">
                        Verify Org.
                    </router-link>
                    <a href="javascript:;" @click="blocked == 1 ? blocked = 2 : blocked = 1" 
                    class="red-btn blocked d-inline py-1">
                        {{ blocked == 1 ? 'Blocked Org.': 'Active Org.'}}</a>
                                ...
                                <div class="btn-group mr-1 mb-1">
                                    ...
                                    <div class="dropdown-menu" x-placement="bottom-start"
                                        ...
                                        <a class="dropdown-item" @click.prevent="blockRow(row.id)"><i
                                                class="fa fa-ban"></i>{{ row.status == 2 ? 'Unblock': 'Block'}}</a>
                                    </div>
                                </div>
                            </td>
                        </tr>
    ...
    export default {
        ...
        methods: {
            getclients() {
                ...
                axios.get(`/organizations?from=${this.dateFormat(this.from)}&to=${this.dateFormat(this.to)}&status=${this.blocked}`)
                    .then(data => {
            ...
            blockRow(id) {
                this.$dialog.confirm(`Are you sure you want to ${this.blocked? 'unblock': 'block'} this organization?`).then(
                    dialog => {
                        axios.delete(`/organizations/${id}`).then(d => {
                            this.getclients();
